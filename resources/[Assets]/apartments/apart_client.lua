...
Citizen.CreateThread(function()
	while true do
		local houseinfo = GetHouseInformation2()
		if houseinfo ~= nil then
			TriggerServerEvent("housing:getGarage", houseinfo[1], houseinfo[2], exports['isPed']:isPed('cid'))
		else
		...
		if backdooroutside["x"] ~= 0.0 then
			...
			if #(vector3(backdooroutside["x"],backdooroutside["y"],backdooroutside["z"]-0.3) - plyCoords) < 1.5 then
				...
				if IsControlJustReleased(2, 38) then
					TriggerServerEvent("house:enterhousebackdoor", house_poi["house_id"], house_poi["house_model"])
					Citizen.Wait(3000)
				end
			end	
		end
		...
		if storage["x"] ~= 0.0 then
			if #(vector3(storage["x"],storage["y"],storage["z"]-0.3) - plyCoords) < 1.5 then
				...
				if IsControlJustReleased(2, 38) then
					...
					TriggerEvent("player:triggeredItemSpawn", "1", "housing-"..house_poi["info"])
...
AddEventHandler('housing:findsalecid', function(checkcid,price,house_id,house_model,storage,clothing,garages)
	...
	while buying > 0 do
		TriggerEvent("DoLongHudText","Your Agreement is a lease-purchase with the Real Estate Company.  Half of total purchase price as down payment must be made upon closing with 5 percent of remaining balance paid per week for a total of 24 weeks. If the client causes any harm to any employee of the Real Estate Company or does not make proper payment for 14 days, this agreement is revoked, and property surrendered.  This clause will be in perpetuity of the leaseholder’s occupation of the property. Type /confirm to confirm.", 91)
		Citizen.Wait(5000)
		buying = buying - 1
	...
	if buy then
		...
		local Player = exports['urp-base']:getModule('LocalPlayer'):getCurrentCharacter()
		if tonumber(Player.cash) >= tonumber(upfront) then
			...
			TriggerEvent('player:removeban', upfront)
		elseif tonumber(Player.bank) >= tonumber(upfront) then
			TriggerServerEvent("house:purchasehouse",tonumber(checkcid),house_id,house_model,upfront,price,housename,storage,clothing,garages,false)
			TriggerEvent('player:passInfoBan', upfront)
		else
...
TriggerServerEvent("house:dopayment", house_id, house_model, playerCash)
					...
					for i=1,#roblist do
						if (GetDistanceBetweenCoords(generator.x + roblist[i]["x"],generator.y + roblist[i]["y"],generator.z + roblist[i]["z"], playerCoords) < 1.4 ) then
							...
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								...
								local finished = exports["urp-taskbar"]:taskBar(15000,"Browsing " ..roblist[i]["name"])
				...
				elseif rentedShop then
					...
					if (#(vector3(curHouseCoords["x"]-1.4,curHouseCoords["y"]-1.47,curHouseCoords["z"]-32.2) - playerCoords) < 2.2 ) then
						...
						if ( not rentedOffices[myhouseid]["trap"]) then
							...
							if IsControlJustReleased(1, Controlkey["housingMain"][1]) then
								TriggerEvent("player:triggeredItemSpawn", "1", PlayerStoreIdentifier)
								Citizen.Wait(5000)
							end
							...
							if (rentedOffices[myhouseid]["owner"] ~= mycid) then
								...
								if IsControlJustReleased(1, Controlkey["generalUse"][1]) then
									local finished = exports["urp-taskbar"]:taskBar(300000,"Doing Takeover")
									if finished == 100 then
								...
								if IsControlJustReleased(1, Controlkey["generalUse"][1]) then
									local finished = exports["urp-taskbar"]:taskBar(10000,"Taking Cash")
									if finished == 100 then
        ...
        if #(GetEntityCoords(PlayerPedId()) - vector3(interiors[i].xe,interiors[i].ye,interiors[i].ze)) < 52 then
            ...
            if #(GetEntityCoords(PlayerPedId()) - vector3(interiors[i].xe,interiors[i].ye,interiors[i].ze)) < 1.599 then
                ...
                if checkDist(distance,t) then
                    ...
                    TriggerEvent("chatMessagess", "Info: ", 1, "You are visiting the " .. interiors[i].name .. " property. " .. interiors[i].price .. "$USD")
        ...
        if #(GetEntityCoords(PlayerPedId()) - vector3(interiors[i].xo,interiors[i].yo,interiors[i].zo)) < 52 then
            ...
            if #(GetEntityCoords(PlayerPedId()) - vector3(interiors[i].xo,interiors[i].yo,interiors[i].zo)) < 1.599 then
                ...
                if  checkDist(distance,t) then
                    ...
                    TriggerEvent("chatMessagess", "Info: ", 4, "You are visiting the " .. interiors[i].name .. " property. " .. interiors[i].price .. "$USD")
...
local apartmentEntries = {
    ...
    [2] = { ["x"] = -265.4687, ["y"] = -963.457, ["z"] = 31.22313, ["name"] = "Alta Apartments Tier 2" },
...
RegisterNetEvent('urp-weapons:stashitem')
AddEventHandler('urp-weapons:stashitem', function(itemnumber,amount,meta)
        ...
        if thename1 ~= "none" then
            ...
            TriggerServerEvent("urp-weapons:stashweapon", thename1, itemnumber, amount, 1,1,meta)
...
RegisterNetEvent('urp-weapons:giveweaponfromproperty')
AddEventHandler('urp-weapons:giveweaponfromproperty', function(result)
...
RegisterNetEvent('urp-weapons:checkinventory')
AddEventHandler('urp-weapons:checkinventory', function()
    ...
    if not IsPedInAnyVehicle(PlayerPedId(),true) then
        ...
        if thename1 ~= "none" then
            ...
            TriggerServerEvent("urp-weapons:checkinventory", thename1)
...
RegisterNetEvent('urp-weapons:takefromstash')
AddEventHandler('urp-weapons:takefromstash', function(itemname, quantity, itemtype, item, askedAmount,meta)
    thename1 = GrabApartment(2)
    if thename1 ~= "none" then
        TriggerServerEvent("urp-weapons:takefromstash",itemname, quantity, itemtype, item, askedAmount,thename1,meta)
    else
        TriggerEvent("DoLongHudText", "Error finding property.",2)
    end
end)
...
RegisterNetEvent('urp-weapons:stashweapon')
AddEventHandler('urp-weapons:stashweapon', function()
    ...
    if not IsPedInAnyVehicle(PlayerPedId(),true) then
        ...
        if thename1 ~= "none" then
            ...
            TriggerEvent("attachWeapons")
            TriggerServerEvent("urp-weapons:stashweapon", thename1, weaponname, 1, 0,ammo)
...
AddEventHandler('AppRobberies:currentlyrobbing', function()
    if exports["urp-inventory"]:hasEnoughOfItem("lockpick",1) then
        checkRobbedStore()
        Citizen.Wait(1500)
