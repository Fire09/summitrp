...
AddEventHandler('hotel:forceOut', function(roomNumber,roomtype)
	...
	if #(vector3(CurrentForced.x, CurrentForced.y, CurrentForced.z) - GetEntityCoords(PlayerPedId())) < 90.0 then
		...
		if roomNumber == 2 then 
			SetEntityCoords(PlayerPedId(),-270.4423583984, -957.50085449219, 31.230158081055)
...
AddEventHandler('hotel:AttemptUpgrade', function()
	if #(vector3(-552.62798583984, -193.59960449219, 38.227691497803) - GetEntityCoords(PlayerPedId())) < 3.0 then
...
AddEventHandler('hotel:createRoom1', function(numMultiplier,roomType,mykeys,illness,isImprisoned,isClothesSpawn)
	...
	TriggerEvent("hotel:myroomtype",myRoomType)
	myspawnpoints  = {
		[1] =  { ['x'] = -256.73,['y'] = -982.53,['z'] = 31.22,['h'] = 199.01, ['info'] = ' Alta Apartments', ["typeSpawn"] = 1 },
	...
	elseif roomType == 2 then
		myspawnpoints[#myspawnpoints + 1] = { ['x'] = -256.73,['y'] = -982.53,['z'] = 31.22,['h'] = 67.09, ['info'] = ' Apartments 2', ["typeSpawn"] = 2 }
	else
...
AddEventHandler('hotel:teleportRoom', function(numMultiplier,roomType)
	...
	if (#(vector3(106.11, -647.76, 45.1) - GetEntityCoords(PlayerPedId())) < 5 and roomType == 3) or (#(vector3(160.26762390137,-641.96905517578,47.073524475098) - GetEntityCoords(PlayerPedId())) < 5 and roomType == 3) or (#(vector3(-270.4423583984, -957.50085449219, 31.230158081055) - GetEntityCoords(PlayerPedId())) < 5 and roomType == 2) then
...
AddEventHandler('newRoomType', function(newRoomType)
	...
	TriggerEvent("hotel:myroomtype",myRoomType)
	SetNewWaypoint(-270.4483691406, -957.59505615234)
...
Citizen.CreateThread(function()
 	...
 	while true do
		...
		local entryUpgradesDst = #(vector3(-552.62798583984, -193.59960449219, 38.227691497803) - plyCoords)
		...
		local entry2nd = #(vector3(-270.4423583984, -957.50085449219, 31.230158081055) - plyCoords)
		...
		if insideApartment or comparedst < 100 then
			...
			if entryUpgradesDst < 5.0 then
				DrawText3Ds(-552.32798583984, -193.91960449219, 38.227691497803, "~g~"..Controlkey["generalUse"][2].."~s~ Upgrade Housing (25k for tier 2.)")
			...
			if (entry4rd < 5 and myRoomType == 3) or (entry3rd < 5 and myRoomType == 3) or (entry1st < 35.0 and myRoomType == 1) or (entry5rd < 65.0 and myRoomType == 1) or (entry2nd < 5 and myRoomType == 2) or (entry6th < 81.0 and myRoomType == 1) then
				...
				if IsControlJustReleased(1,Controlkey["housingMain"][1]) then
					...
					elseif (#(vector3(160.26762390137,-641.96905517578,47.073524475098) - plyCoords) < 5 and myRoomType == 3) or entry4rd < 5 then
						processBuildType(myRoomNumber,myRoomType)			
					elseif #(vector3(-270.4423583984, -957.50085449219, 31.230158081055) - plyCoords) < 5 and myRoomType == 2 then
						processBuildType(myRoomNumber,myRoomType)
					else
...
Citizen.CreateThread(function()
    exports["srp-polyzone"]:AddBoxZone("apartment_enter", vector3(-270.44, -957.5, 31.23), 3.6, 3, {
