...
AddEventHandler("server:enablehuddebug", function(enable)
    ...
    local src = source
    if debug then
        exports["srp-log"]:AddLog("Admin", GetPlayerName(src), "Dev Debug", {item = tostring("Enabled")}) 
        TriggerClientEvent('hud:enabledebug', src)
    else
        exports["srp-log"]:AddLog("Admin", GetPlayerName(src), "Dev Debug", {item = tostring("Disabled")}) 
        TriggerClientEvent('hud:enabledebug', src)
    end
end)
