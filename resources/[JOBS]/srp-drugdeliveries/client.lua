...
local carspawns = {
	[1] =  { ['x'] = 499.16718261719,['y'] = -635.2993212891,['z'] = 24.487556381226,['h'] = 262.760859375, ['info'] = ' car 8' },
	[2] =  { ['x'] = 499.16718261719,['y'] = -635.2993212891,['z'] = 24.487556381226,['h'] = 262.760859375, ['info'] = ' car 1' },
	[3] =  { ['x'] = 499.16718261719,['y'] = -635.2993212891,['z'] = 24.487556381226,['h'] = 262.760859375, ['info'] = ' car 2' },
}
...
Citizen.CreateThread(function()
	exports["srp-polyzone"]:AddBoxZone("oxy_run_start", vector3(500.61, -651.55, 24.75), 3, 3, {
		name="oxy_run_start",
		heading=35,
		--debugPoly=true,
		minZ=23.99,
...
AddEventHandler('srp-polyzone:enter', function(name)
    if name == "oxy_run_start" then
		...
		if not OxyRun then
			...
			TriggerEvent('srp-ui:ShowUI', 'show', ("%s"):format("Come Back Later")) 
		end
    end
end)
...
function DoDropOff(requestMoney)
	...
	cashPayment = 300 + math.random(1000)
	local success = true
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
			...
			if OxyRun then
				...
				cashPayment = math.random(90,325)
				
				if exports["srp-inventory"]:hasEnoughOfItem("inkedmoneybag",1,false) then     

		        elseif exports["srp-inventory"]:hasEnoughOfItem("none",10,false) then     
		            TriggerEvent("inventory:removeItem","none", 10)   
				...
				elseif exports["srp-inventory"]:hasEnoughOfItem("none",10,false) then     
		            ...
		            TriggerEvent("DoLongHudText","Thanks, Stop trippin tho?!")
		        end

				if math.random(100) > 45 then
					TriggerEvent( "player:receiveItem", "oxy", math.random(5,9))
				end
				cashPayment = math.random(100, 400)
		end
	end
	...
	if success then
		...
		TriggerEvent("client:newStress",true,350)
