...
AddEventHandler("docks:manage", function(arg,arg2)
  local rank = exports["isPed"]:GroupRank("all")
  if rank > 3 then
    if arg == "list" then
...
local locations = {
  [1] = {-608.28, -2357.78, 13.74}
}

local garges = {
[1] =  { -608.28, -2357.78, 13.74, 319.75,true,0}, 

[2] =  { -597.98, -2366.95, 13.74, 318.52,true,0}, 
    ...
    if exports["isPed"]:isPed("myjob") and existingVeh ~= nil then
      ...
      elseif job ~= 0 and stage == "drop" then
        ...
        elseif currentJobs[currentJobPos].JobType ~= "Shops" then
          ...
          if dist < 20 then
            ...
            if payment == nil or payment < 600 then
              payment = 600
            end
            ...
            payment = math.random(600,payment)
      ...
      elseif job ~= 0 and stage == "waiting" then
        if waitingForConfirmation then
          ...
          if math.random(200) > 266 then
            --
            TriggerEvent("payment:chopshopscrap",math.random(30),false)
          end

          TriggerServerEvent("server:givepayJob", "Delivery Service", math.random(200,payment))
        ...
        if #(vector3(v[1],v[2],v[3]) - GetEntityCoords(PlayerPedId())) < 5.5 then
          ...
          if(IsControlJustPressed(1, Controlkey["generalUse"][1])) then
            ...
            if job ~= 0 then
              ...
              TriggerEvent("DoLongHudText","You need to Sign on as trucker Open your phone to select a available delivery.",2)
            end
