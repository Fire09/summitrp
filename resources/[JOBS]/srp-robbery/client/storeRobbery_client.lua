...
AddEventHandler("robbery:register", function()
	...
	TriggerEvent("player:receiveItem","rollcash",math.random(30,90))
end)
...
AddEventHandler("robbery:safe", function()
	...
	if tonumber(storeid) == tonumber(CorrectStore) then
		...
		TriggerEvent("player:receiveItem","rollcash",math.random(100, 120))

	end

	if math.random(25) > 5 then
    	TriggerEvent("player:receiveItem","G6 C Card",1)
	end

	TriggerEvent("player:receiveItem","band",math.random(25, 50))
...
Citizen.CreateThread(function()
	exports["srp-polyzone"]:AddBoxZone("store_robbery_lucky", vector3(1163.3, -457.1, 71.88), 3, 2, {
