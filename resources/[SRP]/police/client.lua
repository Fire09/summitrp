...
AddEventHandler('police:getArrested2', function(cuffer)
	...
	if not isDead and cuffAttemptCount < 2 then
		...
		finished = exports['srp-taskbarskill']:taskBar(-6000, 2)
	end
	...
	if job == "auto_bodies" or job == 'paleto_mech' or job == 'auto_exotics' or job == 'tuner_shop' or job == "ems" or job == "police" or job == "car_shop" then
		...
		if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
			...
			if finished == 100 then
				...
				FreezeEntityPosition(PlayerPedId(),false)
				deleteVeh(vehicle)
				TriggerEvent("DoShortHudText","Vehicle Impounded")
			else
				FreezeEntityPosition(PlayerPedId(),false)
				ClearPedTasks(PlayerPedId())
			end
...
RegisterNetEvent('impoundscuff')
AddEventHandler('impoundscuff', function()
		coordA = GetEntityCoords(PlayerPedId(), 1)
		coordB = GetOffsetFromEntityInWorldCoords(PlayerPedId(), 0.0, 100.0, 0.0)
		vehicle = getVehicleInDirection(coordA, coordB)
		if DoesEntityExist(vehicle) and NetworkHasControlOfEntity(vehicle) then
			TriggerEvent("animation:impound")
			FreezeEntityPosition(PlayerPedId(),true)
			local finished = exports["srp-taskbar"]:taskBar("3000","Deleting Scuffed Vehicle")
			if finished == 100 then
				ClearPedTasks(PlayerPedId())
				FreezeEntityPosition(PlayerPedId(),false)
				deleteVeh(vehicle)
				TriggerEvent("DoShortHudText","Thanks for making city better!")
			else
				FreezeEntityPosition(PlayerPedId(),false)
				ClearPedTasks(PlayerPedId())
			end
