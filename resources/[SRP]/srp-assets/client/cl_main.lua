...
AddEventHandler('rich:TakePlayers', function(count)
    ...
    if SetDiscordRichPresenceAction then
        SetDiscordRichPresenceAction(0, 'Click To Apply', '')
    end
end)

Citizen.CreateThread(function()
    TriggerServerEvent('rich:GetPlayers')
    SetDiscordAppId(tonumber(GetConvar("RichAppId", "")))
    SetDiscordRichPresenceAsset(GetConvar("RichAssetId", "srp"))
    while true do
        Citizen.Wait(2000)
        SetDiscordRichPresenceAssetText("")
        SetRichPresence(onlinePlayers.."/64 Players")
    end
end)
...
Citizen.CreateThread(function()
    while true do
    	...
    	Wait(0)
        N_0x4757f00bc6323cfe(GetHashKey("WEAPON_CARBINERIFLE_MK2"), 0.97)
    	Wait(0)
        N_0x4757f00bc6323cfe(GetHashKey("WEAPON_ASSAULTRIFLE"), 0.73)
    	Wait(0)
        N_0x4757f00bc6323cfe(GetHashKey("WEAPON_ASSAULTRIFLE_MK2"), 0.97)
    	Wait(0)
