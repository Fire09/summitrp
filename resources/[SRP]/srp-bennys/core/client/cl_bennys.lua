...
local bennyLocationTuner = vector3(938.37, -970.82, 36.99)
local billyLocationPaleto = vector3(1174.78, 2639.68, 37.75)
...
local mrpd = vector3(450.14, -975.91, 25.7)

---local civrepairbennys = vector3(-33.53, -1053.58, 28.05) -- Repair shop for civs in bennys
...
AddEventHandler('event:control:bennys', function(useID)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        bennyHeading = 180.73135375977
        if useID == 1 and not isPlyInBennys and exports["isPed"]:isPed("myJob") == "tuner_shop" then -- Bennys harmon
    ...
    while true do 
        ...
        if IsPedInAnyVehicle(plyPed, false) then
            ...
            if nearTuner or nearPaleto or nearHayes or nearAutoExotics then
                ...
                -- harmony
                if nearPaleto and job == "tuner_shop" then
                    if not isPlyInBennys then
                        Draw3DText(billyLocationPaleto.x, billyLocationPaleto.y, billyLocationPaleto.z + 0.5, "[Press ~p~E~w~ - Enter Harmony Performance]", 255, 255, 255, 255, 4, 0.45, true, true, true, true, 0, 0, 0, 0, 55)
...
RegisterNetEvent('civ:quickfix')
AddEventHandler('civ:quickfix', function()
    Quickfix()
end)

function Quickfix()
	local ped = GetPlayerPed(-1)
	for k,v in pairs(quickfixx) do 
...
quickfixx = {
    {-39.53066040039,-1052.423089294434,28.480166320801},
}
