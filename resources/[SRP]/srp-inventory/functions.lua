...
--    end  
    ...
    if (itemid == "radio") or (itemid == "civradio") then
    ...
    if (itemid == "pix2") then
      if (#(GetEntityCoords(player) - vector3( 1275.49, -1710.39, 54.78)) < 3.0) then
          local finished = exports["srp-taskbar"]:taskBar(25000,"Decrypting Data",false,false,playerVeh)
          if (finished == 100) then
            TriggerEvent("oxyrunesx:startOxyRun", source)
            remove = true
          end
      end
    end
    ...
    if (itemid == "pixellaptop")  then
        TriggerServerEvent("bropixel-boosting:usedlaptop")
    end    

    if (itemid == "disabler")  then
        TriggerServerEvent("bropixel-boosting:useddisabler")
    end
    ...
    if (itemid == "femaleseed") then
        if hasEnoughOfItem('plantpot', 1) then
            ...
            TriggerEvent('srp-weed:startcrop', 'female')
        end
    end
    ...
    if (itemid == "maleseed") then
        if hasEnoughOfItem('plantpot', 1) then
            ...
            TriggerEvent('inventory:removeItem', 'plantpot', 1)
            TriggerEvent('srp-weed:startcrop', 'male')
        end
    end

    if (itemid == "heistusb4") then
        TriggerServerEvent("srp-fleeca:usb",itemid)
        TriggerEvent('srp-fleeca:usb')
    end
    ...
    if (itemid == "chips" or itemid == "chips") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","food:Condiment",true,itemid,playerVeh)
    end

    if (itemid == "cbveggy" or itemid == "cbveggy") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","food:Condiment",true,itemid,playerVeh)
    end

    if (itemid == "cookie" or itemid == "cookie") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","food:Condiment",true,itemid,playerVeh)
    end

    if (itemid == "chocobar" or itemid == "Chocolate Bar") then
        TaskItem("mp_player_inteat@burger", "mp_player_int_eat_burger", 49,6000,"Eating","food:Condiment",true,itemid,playerVeh)
    end
    ...
    if (itemid == "widowmakerammo") then
        ...
        if (finished == 100) then
            TriggerEvent("actionbar:ammo",1213462521,2,true)
            remove = true
        end
    end
    ...
    if (itemid == "pistolammoPD") then
        local finished = exports["srp-taskbar"]:taskBar(2500,"Reloading",false,false,playerVeh)
        if (finished == 100) then
            TriggerEvent("actionbar:ammo",1950175060,50,true)
            remove = true
        end
    ...
    if (itemid == "empammo") then
        local finished = exports["srp-taskbar"]:taskBar(30000,"Recharging EMP",false,false,playerVeh)
        if (finished == 100) then
    ...
    if (itemid == "Smallsupplycrate") then
        if (hasEnoughOfItem("2227010557",1,false)) then
            ...
            if (finished == 100) then
                remove = true
                TriggerEvent("inventory:removeItem","2227010557", 1) 
                TriggerEvent("phone:addnotification", "Gun Dealer","<b>Gun Dealer</b> <br>Be careful with this shit!</br>")

                TriggerEvent( "player:receiveItem", "1649403952", 1 )
                TriggerEvent( "player:receiveItem", "rifleammo", 5 )
            end
        else
            TriggerEvent("DoLongHudText","You are missing something to open the crate with",2)
        end
    ...
    if (itemid == "backpack") then
        local finished = exports["srp-taskbar"]:taskBar(7500,"Unzipping The Goodies!")
        if (finished == 100) then
            remove = true
            TriggerServerEvent("backpack:give:items")
            TriggerEvent("DoLongHudText","Type /help To See SRP Guide",1)
        end
    end
    ...
    if (itemid == "nitrous") then
        ...
        if not IsToggleModOn(currentVehicle,18) then
            TriggerEvent("DoLongHudText","You need to install turbo!",2)
        else
            local finished = 0
