const mdtApp = new Vue({
    ...
    methods: {
        ...
        AttachToCall(index) {
            $.post('http://srp-mdt/attachToCall', JSON.stringify({
                index: index,
        ...
        DetachFromCall(index) {
            $.post('http://srp-mdt/detachFromCall', JSON.stringify({
                index: index
            }));
            this.current_call = {
                source: null,
        ...
        SetCallWaypoint(index) {
            $.post('http://srp-mdt/setCallWaypoint', JSON.stringify({
                coords: this.calls[index].coords
            }));
            return;
        },
        ...
        SaveEditCall() {
            $.post('http://srp-mdt/editCall', JSON.stringify({
        ...
        DeleteCall(index) {
            $.post('http://srp-mdt/deleteCall', JSON.stringify({
                index: index
            }));
            return;
        },
        ...
        window.addEventListener('message', function(event) {
            if (event.data.type == "enable") {
                ...
                if (mdtApp.current_call.id == mdtApp.calls[event.data.call].id) {
                    $.post('http://srp-mdt/deleteCallBlip')
                    mdtApp.current_call = {
                        source: null,
                        details: null,
                        id: null,
