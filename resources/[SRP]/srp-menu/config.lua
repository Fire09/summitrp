...
rootMenuConfig =  {
    ...
    },
    {
        ...
        subMenus = {"general:escort", "cuffs:remmask", "cuffs:checkinventory", "police:gsr", "police:dnaswab", "general:checktargetstates", "police:checkbank", "police:placespike", "police:removespike"}
    },
    {
        id = "police-license",
    ...
    },
    {
        ...
        subMenus = {"police:runplate", "police:toggleradar", "Repair"}
    },
    {
        id = "policeDeadA",
        displayName = "10-13A",
...
-- return (not isDead and isJudge)
   ...
   --  },
...

    {
        ...
        icon = "#fix",
        functionName = "fix:police",
        enableMenu = function()
            for k,v in ipairs(vehiclefix) do
                local pos = v.loc
                if #(vector3(pos[1],pos[2],pos[3]) - GetEntityCoords(PlayerPedId())) < 3.0 and not isDead and isPolice and IsPedInAnyVehicle(PlayerPedId(), false) then
                    return true
                end
            end
            return false
...

    {
        ...
        displayName = "Quick Fix",
        icon = "#fix",
        functionName = "civ:quickfix",
        enableMenu = function()
            for k,v in ipairs(quickfix) do
                local pos = v.loc
                if #(vector3(pos[1],pos[2],pos[3]) - GetEntityCoords(PlayerPedId())) < 3.0 and not isDead and IsPedInAnyVehicle(PlayerPedId(), false) then
                    return true
                end
            end
            return false
...

    {
        ...
        displayName = "Quick Fix",
        icon = "#fix",
        functionName = "fix:ems",
        enableMenu = function()
            for k,v in ipairs(vehiclefix) do
                local pos = v.loc
                if #(vector3(pos[1],pos[2],pos[3]) - GetEntityCoords(PlayerPedId())) < 3.0 and not isDead and isMedic and IsPedInAnyVehicle(PlayerPedId(), false) then
                    return true
                end
            end
            return false
...

    {
        ...
        enableMenu = function()
            ...
            local distance = GetDistanceBetweenCoords(plyCoords.x,plyCoords.y,plyCoords.z,2996.46088134766, 2754.3775, 43.12809265137,false)
    ...
    },
    {
        ...
        enableMenu = function()
            return ((isPolice or exports["isPed"]:isPed("myjob") == "police") and not isDead)
        end
    }
...
vehiclefix = {
	[1] = { loc = {450.52066040039,-975.993089294434,25.380166320801}},
    [2] = { loc = {339.34066040039,-571.283089294434,28.330166320801}},
}

quickfix = {
	[1] = { loc = {-39.53066040039,-1052.423089294434,28.480166320801}},
}