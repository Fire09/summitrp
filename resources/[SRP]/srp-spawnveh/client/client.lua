...
RegisterNetEvent('fix:police')
AddEventHandler('fix:police', function()
    if exports["isPed"]:isPed("myjob") == "police" then
	    policeFix()
	end
end,false)

RegisterNetEvent('fix:ems')
AddEventHandler('fix:ems', function()
...
------ NETEVENTS ------
-----------------------
RegisterNetEvent('ImpoundVehicles')
AddEventHandler('ImpoundVehicles', function()
	TriggerEvent('srp-context:sendMenu', {
		...
		{
            id = 3,
            header = "Delete Scuffed Vehicles",
			txt = "Delete NPC Scuffed Vehicles / ONLY FOR NPC CARS",
			params = {
                event = "impoundscuff",
            }
        },
...
RegisterNetEvent('PoliceContext')
AddEventHandler('PoliceContext', function()
	TriggerEvent('srp-context:sendMenu', {
        {
            id = 1,
            header = "Vehicles",
			txt = "PD Owned Vehicles",
			params = {
                event = "PDSpawnVeh"
            }
        },
        {
            id = 2,
            header = "Police Catalog",
			txt = "Purchase your own vehicle",
			params = {
                event = "PoliceContextCatalog"
            }
        },
    })
end)

RegisterNetEvent('PoliceContextCatalog')
AddEventHandler('PoliceContextCatalog', function()
	TriggerEvent('srp-context:sendMenu', {
		{
            id = 1,
            header = "< Back",
		...
		{
            id = 2,
            header = "Police Ford Taurus",
			txt = "$5,000",
			params = {
                event = "police:buypoltaurus"
            }
        },
        {
            id = 3,
            header = "Police Ford Crown Victoria",
			txt = "$10,000",
			params = {
...
AddEventHandler("spawn:hei:ems", function()
	Citizen.CreateThread(function()
        ...
        local hash = GetHashKey("emsaw139")
...
function SpawnVehPD(name)
	Citizen.CreateThread(function()
        ...
        local vehicle = CreateVehicle(hash, 458.96408935547, -992.77743896484, 25.38979095459, 1.190180053711, true, false)
...
function policeFix()
	...
	for k,v in pairs(vehiclefixpolice) do 
		if IsPedInAnyVehicle(ped, true) then
			local veh = GetVehiclePedIsIn(ped, false)
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= 3.0 then
				TriggerEvent('DoLongHudText', 'Your vehicle is being repaired please wait', 1)
				local finished = exports["srp-taskbar"]:taskBar(1000, "Repairing")
...
function EMSFix()
	local ped = GetPlayerPed(-1)
	for k,v in pairs(vehiclefixems) do 
		if IsPedInAnyVehicle(ped, true) then
			local veh = GetVehiclePedIsIn(ped, false)
			if GetDistanceBetweenCoords(GetEntityCoords(ped), v[1], v[2], v[3], true) <= 3.0 then
				TriggerEvent('DoLongHudText', 'Your vehicle is being repaired please wait', 1)
