    ...
    Citizen.CreateThread(function()
        ...
        while true do
            ...
            if isVoiceActive and not VoiceEnabled then
                TriggerEvent('srp:voice:state', true)
            elseif not isVoiceActive and VoiceEnabled then
                TriggerEvent('srp:voice:state', false)
...
AddEventHandler('srp:voice:state', function(state)
    VoiceEnabled = state

    -- exports['tcm_hud']:toggleMumbleWarning(state)

    TriggerServerEvent("srp:voice:connection:state", state)
...
function AddPlayerToTargetList(serverID, context, transmit)
    if not Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("srp:voice:transmission:state", serverID, context, true, false)
        end
...
function RemovePlayerFromTargetList(serverID, context, transmit, refresh)
    if Targets:targetContextExist(serverID, context) then
        ...
        if transmit then
            TriggerServerEvent("srp:voice:transmission:state", serverID, context, false, false)
        end
...
function AddGroupToTargetList(group, context)
    ...
    TriggerServerEvent("srp:voice:transmission:state", group, context, true, true)
end
...
function RemoveGroupFromTargetList(group, context)
    ...
    TriggerServerEvent("srp:voice:transmission:state", group, context, false, true)
end
...
function SetVoiceProximity(proximity)
    ...
    TriggerEvent("srp-hud:changeRange", proximity)
...
RegisterNetEvent("srp:voice:transmission:state")
AddEventHandler("srp:voice:transmission:state", function(serverID, context, transmitting)
...
RegisterNetEvent('srp:voice:targets:player:add')
AddEventHandler('srp:voice:targets:player:add', AddPlayerToTargetList)

RegisterNetEvent('srp:voice:targets:player:remove')
AddEventHandler('srp:voice:targets:player:remove', RemovePlayerFromTargetList)
...
Citizen.CreateThread(function()
    ...
    SetSettings(exports["srp-base"]:getModule("SettingsData"):getSettingsTable())
end)
