...
function addPlayerToCall(source, channel)
    print("[srp-voice]PLAYER ON PHONE ", source, channel)
    ...
    for player, _ in pairs(callData[channel]) do
        print("[srp-voice]PLAYER ON PHONE 2 ", source, channel)
    ...
    print("[srp-voice]syncCallData ", source, json.encode(callData[channel]))
    TriggerClientEvent('srp-voice:syncCallData', source, callData[channel])
end
...
function setPlayerCall(source, callChannel)
    print('[srp-voice:setPlayerCall] SetPlayer', source, callChannel)
