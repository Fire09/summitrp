...
RegisterNetEvent('srp-weapons:destroyGroundWeapon')
AddEventHandler('srp-weapons:destroyGroundWeapon', function()
	DeleteEntity(weapon)
end)

RegisterNetEvent('srp-weapons:allowDrop')
AddEventHandler('srp-weapons:allowDrop', function()
	canDrop = true
end)

RegisterNetEvent('srp-weapons:dropweapon')
AddEventHandler('srp-weapons:dropweapon', function(AmountOfCurrentWeapon,weaponModel,rowID)
    ...
    canDrop = false
	TriggerServerEvent("srp-weapons:dropWeapon",{pos.x,pos.y,pos.z+0.75},name,rowID)
...
RegisterNetEvent('srp-weapons:doHandOver')
AddEventHandler('srp-weapons:doHandOver', function(weapon)
  --GiveWeaponToPed(PlayerPedId(), tonumber(weapon), 0, 0, false)
  TriggerEvent("attachWeapons")
end)

RegisterNetEvent('srp-weapons:dropWeaponSearch')
AddEventHandler('srp-weapons:dropWeaponSearch', function(weapon)
	--RemoveWeaponFromPed(PlayerPedId(),tonumber(weapon))
  TriggerEvent("attachWeapons")
end)


RegisterNetEvent('srp-weapons:giveweapon')
AddEventHandler('srp-weapons:giveweapon', function(toPlayer,AmountOfCurrentWeapon,weaponModel,rowID)
	...
	if not IsPedInAnyVehicle(PlayerPedId(),true) then
		...
		if not IsNearPlayer(toPlayer) then TriggerEvent('chatMessage', "", 1, "^1You are not near this player!") return end
		...
		if Vdist2(targetPos, userCoords) > 15.0 then
			TriggerEvent('chatMessage', "", 1, "^1You are not near this player!")
			return
		end
		...
		if (playing ~= false) then
			...
			TriggerServerEvent("srp-weapons:giveweapon", toPlayer, name,rowID)
			Wait(1300)
			TriggerServerEvent("weaponshop:removeSingleWeapon",rowID)
			TriggerEvent("attachWeapons")
		else
			TriggerEvent('chatMessage', "", 1, "^1This player is not online!")
		end
...
AddEventHandler('attachWeapons', function()
	...
	local currentWeapons = exports["srp-inventory"]:GetCurrentWeapons() 
	weapcount = 1
	local curw = GetSelectedPedWeapon(PlayerPedId())
...
AddEventHandler('DisplayWeapons', function(results) 
	...
	TriggerEvent('chatMessage', 'SEARCH - WEAPONS', 1, " TOTAL: (" .. #results .. ")")
	...
	for k,v in ipairs(results) do
		...
		if tostring( hashGunToText[curCheck] ) ~= nil then
			...
			if v.originalCID == nil or v.originalCID == 0 then
				TriggerEvent('chatMessage', 'SEARCH - WEAPONS', 1, hashGunToText[curCheck] .. " - (Unknown) ".." - Cartridge: "..cartridge)
			else
				if hashGunToText[curCheck] == nil then
					TriggerEvent('chatMessage', 'SEARCH - WEAPONS', 1, "Error: Unknown Weapon - (" .. v.originalCID .. ")".." - Cartridge: "..cartridge)
				else
					TriggerEvent('chatMessage', 'SEARCH - WEAPONS', 1, hashGunToText[curCheck] .. " - (" .. v.originalCID .. ")".." - Cartridge: "..cartridge)
				end				
			end	
		else
			TriggerEvent('chatMessage', 'SEARCH - WEAPONS', 1, v.weapon_model .. " - (" .. v.originalCID .. ")".." Cartridge: "..cartridge)
		end
		if isMelee then
			for _,j in ipairs(metaData.dna) do
				TriggerEvent('chatMessage', 'Evidence - WEAPONS', 1, hashGunToText[curCheck] .." - Dna: "..j)
			end
		end
